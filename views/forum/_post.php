...
<div class = "post">
	...
	<div class = "member-cell">
		<div class = "membername">
			<?php echo Html::link(Html::encode($data->poster->member_name), array('member/view', 'id' => $data->poster->id)); ?>
		</div>
		<div class = "avatar">
			<?php echo Html::image((isset($data->poster->avatar))?(Yii::$app->request->baseUrl . $this->module->avatarStorage . '/'. $data->poster->avatar):$this->module->getRegisteredImage('empty.jpeg'), 'avatar'); ?>
		</div>
		<div class = "group">
			<?php if(isset($data->poster->group)) {
				if(isset($data->poster->group->image)) {
					echo Html::image($this->module->getRegisteredImage($data->poster->group->image), 'group') . '<br>';
				}
		...
		<?php if(!Yii::$app->user->isGuest): ?>
			<?php echo Html::image($this->module->getRegisteredImage('warn.png'), 'warn', array('title' => Yii::t('BbiiModule.bbii', 'Report post'), 'style' => 'cursor:pointer;', 'onclick' => 'reportPost(' . $data->id . ')')); ?>
			<?php echo Html::link( Html::image($this->module->getRegisteredImage('pm.png'), 'pm', array('title' => Yii::t('BbiiModule.bbii', 'Send private message'))), array('message/create', 'id' => $data->user_id) ); ?>
	...
	<div class = "post-cell">
		...
		<?php if($this->context->isModerator()): ?>
			<?php echo Html::link( Html::image($this->module->getRegisteredImage('warn.png'), 'warn', array('title' => Yii::t('BbiiModule.bbii', 'Warn user'))), array('message/create', 'id' => $data->user_id, 'type' => 1) ); ?>
			<?php echo Html::image($this->module->getRegisteredImage('delete.png'), 'delete', array('title' => Yii::t('BbiiModule.bbii', 'Delete post'), 'style' => 'cursor:pointer;', 'onclick' => 'if(confirm("' . Yii::t('BbiiModule.bbii','Do you really want to delete this post?') . '")) { deletePost("' . $this->createAbsoluteUrl('moderator/delete', array('id' => $data->id)) . '") }')); ?>
			<?php echo Html::image($this->module->getRegisteredImage('ban.png'), 'ban', array('title' => Yii::t('BbiiModule.bbii', 'Ban IP address'), 'style' => 'cursor:pointer;', 'onclick' => 'if(confirm("' . Yii::t('BbiiModule.bbii','Do you really want to ban this IP address?') . '")) { banIp(' . $data->id . ', "' . $this->createAbsoluteUrl('moderator/banIp') . '") }')); ?>
		<?php endif; ?>
		</div>
	</div>
</div>